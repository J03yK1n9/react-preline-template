...
function Footer({ links }) {
  return (
    <>
  {/* ========== FOOTER ========== */}
  <footer className="mt-auto w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto dark:bg-custom_dark">
    {/* Grid */}
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-10">
      ...
      <div className="grid col-span-full grid-cols-2 lg:grid-cols-4 lg:col-span-7 lg:mb-10 lg:gap-6 md:justify-items-center">
        {links.map(link => {
          return (
            <div className="" key={link.title || link.to}>
              {link.title && <h4 className="text-xs font-semibold text-gray-900 uppercase dark:text-neutral-100">{link.title}</h4>}
              {link.links && link.links.map(sublink => (
                <div className="mt-3 grid space-y-3 text-sm" key={sublink.to}>
                  <p>
                    <CustomLink 
                      className="inline-flex gap-x-2 text-gray-600 hover:text-gray-800 dark:text-neutral-400 dark:hover:text-neutral-200" 
                      url={sublink.to}>
